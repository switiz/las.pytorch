...
def load_model(opt, model, vocab):
    start_epoch = 0
    optimizer = torch.optim.Adam(params=model.parameters(), lr=3e-4, weight_decay=1e-05)
    criterion = LabelSmoothingLoss(len(vocab), ignore_index=vocab.pad_id, smoothing=opt['label_smoothing']).cuda()
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0003,
                                                  step_size_up=5, max_lr=0.001,
